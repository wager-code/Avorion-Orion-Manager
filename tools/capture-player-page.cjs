...
const sourcePath = path.join(projectRoot, "references", "玩家管理_方案3_已确认.png");
const implementationPath = path.join(outputDirectory, "player-management-implementation-1672x941.png");
const comparisonPath = path.join(outputDirectory, "player-management-comparison-full.png");
const focusedComparisonPath = path.join(outputDirectory, "player-management-comparison-focused.png");
...
(async () => {
  ...
  const initialRows = await page.locator(".players-table tbody tr").count();
  const eventRows = await page.locator(".players-events li").count();
  ...
  const serverNavigation = page.getByText("服务器管理", { exact: true });
  if (await serverNavigation.count()) {
    await serverNavigation.first().click();
    await page.waitForURL(/\/server\/control$/, { timeout: 5_000 });
  ...
  for (const viewport of [{ width: 1366, height: 768 }, { width: 1024, height: 768 }, { width: 768, height: 900 }]) {
    ...
    await responsivePage.goto("http://127.0.0.1:4173/players", { waitUntil: "networkidle", timeout: 60_000 });
  ...
  </style><main><section class="panel"><div class="label">视觉基准：方案 3</div><img src="${sourceUrl}"></section><section class="panel"><div class="label">当前实现：玩家管理</div><img src="${implementationUrl}"></section></main>`);
  ...
  await focused.setContent(`<!doctype html><style>
    *{box-sizing:border-box}body{margin:0;background:#0c1524;color:#fff;font-family:Segoe UI,sans-serif}
    ...
    .crop{height:648px;background-repeat:no-repeat;background-size:1672px 941px;background-position:-286px -92px;background-color:#eef3f9}
  </style><main><section class="panel"><div class="label">方案 3 · 主工作区</div><div class="crop" style="background-image:url('${sourceUrl}')"></div></section><section class="panel"><div class="label">当前实现 · 主工作区</div><div class="crop" style="background-image:url('${implementationUrl}')"></div></section></main>`);
  await focused.waitForTimeout(800);
  ...
  console.log(JSON.stringify({
    ...
    detailOpened,
    navigationRoundTrip,
    playerVisibleInitially: initialBody.includes("超人特工队"),
    hasLiveConnection: initialBody.includes("实时连接"),
    hasRealSource: initialBody.includes("服务端实时"),
    consoleErrors,
    pageErrors,
    responsiveChecks,
    implementationPath,
