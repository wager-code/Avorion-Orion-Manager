...
for (const viewport of [
  ...
  await page.getByText("Avorion Dedicated Server 已验证：", { exact: true }).waitFor();
  ...
  const actionVisible = await page.getByRole("button", { name: "验证路径并继续配置", exact: true }).isVisible();
  const screenshot = path.join(outputDir, `01-real-install-success-${viewport.name}.png`);
  await page.screenshot({ path: screenshot, fullPage: true });
  report.screenshots.push(screenshot);
  report.checks.push({ viewport: viewport.name, actionVisible, ...bodyMetrics, visibleTextOverflows });
  await context.close();
}
